---
title: Homework #4, Due Friday, March 9th 
---
...
>   where parseVar = do x <- consumeP $ varP
>                       return $ Var x
>         parseVal = do x <- consumeP $ valueP
>                       return $ Val x
>         parseOp x  =  grab x <|> return x
>         grab  x   =  do o <- consumeP $ opP
>                         y <- exprP
>                         return $ Op o x y
>         parseParent =  do consumeP $ char '('
>                           y <- exprP
>                           consumeP $ char ')'
>                           return y
...
> consumeP p = do spaces
>                 x <- p
>                 spaces >> return x

> statementP :: Parser Statement
> statementP = skip <|> ((parseAss <|> parseIf <|> parseWhile) >>= parseSeq)
>   where skip = constP "skip" Skip
>         parseAss = do x <- varP
>                       consumeP $ string ":="
>                       e <- exprP
>                       return (Assign x e)
>         parseIf  = do consumeP $ string "if"
>                       x <- exprP
>                       consumeP $ string "then"
>                       s1 <- statementP
>                       consumeP $ string "else"
>                       s2 <- statementP
>                       consumeP $ string "endif"
>                       return (If x s1 s2)
>         parseWhile = do consumeP $ string "while"
>                         x <- exprP
>                         consumeP $ string "do"
>                         s <- statementP
>                         consumeP $ string "endwhile"
>                         return (While x s)
>         parseSeq x = grab x <|> return x
>         grab x = do consumeP $ char ';'
>                     s <- statementP
>                     return (Sequence x s)
...
> convOp :: Bop -> Value -> Value -> Value
> convOp Plus (IntVal x) (IntVal y) = IntVal (x + y)
> convOp Minus (IntVal x) (IntVal y) = IntVal (x - y)
> convOp Times (IntVal x) (IntVal y) = IntVal (x * y)
> convOp Divide (IntVal x) (IntVal y) = IntVal (x `div` y)
> convOp Gt (IntVal x) (IntVal y) = BoolVal (x > y)
> convOp Ge (IntVal x) (IntVal y) = BoolVal (x >= y)
> convOp Lt (IntVal x) (IntVal y) = BoolVal (x < y)
> convOp Le (IntVal x) (IntVal y) = BoolVal (x <= y)
...
>                                       put $ insert x v m
>                                       return ()
>                 Sequence s1 s2  -> evalS s1 >> evalS s2
>                 w@(While e s)   -> evalS (If e (Sequence s w) Skip)
>                 If e s1 s2      -> do v <- evalE e
>                                       case v of 
>                                         BoolVal True  -> evalS s1
>                                         BoolVal False -> evalS s2
>                                         _             -> return ()
>                 Print s e       -> do r <- evalE e
>                                       tell $ s ++ show r ++ "\n"
>                                       return ()
>                 Throw e         -> do r <- evalE e
>                                       throwError r
>                                       return ()
>                 Try s1 v s2     -> do { evalS s1 }`catchError` (\err ->  do m <- get
>                                                                             put $ insert v err m
>                                                                             evalS s2)
>               where evalE (Var v)   =  do m <- get
>                                           if notMember v m
>                                             then throwError (IntVal 0)
>                                             else return $ m ! v
...
>                                                                 else return $ convOp o l r
>                                                     otherwise -> return $ convOp o l r
> 
> checkType (IntVal _) (BoolVal _) = True
> checkType (BoolVal _) (IntVal _) = True
> checkType _ _ = False

and use the above function to implement a second function

> execute :: Store -> Statement -> (Store, Maybe Value, String)
> execute st stmt = (cnt, exn, log)
>   where ((res, cnt), log) = runWriter (runStateT (runErrorT m) st)
>         m = evalS stmt
>         exn = case res of
>                 Left s -> Just s
>                 Right _ -> Nothing
...
> testprog0 = mksequence [Assign "X" $ Val $ IntVal 0,
>                         Assign "Y" $ Val $ IntVal 1,
>                         Print "hello world: " $ Var "X",
>                         If (Op Lt (Var "X") (Var "Y")) Skip
>                                                        Skip,
...
> prop_bitSubtractor_Correct s l1 = error "TODO"
